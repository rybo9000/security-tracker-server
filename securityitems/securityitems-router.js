...
SecurityItemsRouter.route("/")
  ...
  .post(jsonParser, (req, res, next) => {
    ...
    CategoriesService.getCategoryById(knexInstance, category).then(
      (results) => {
        if (results.length === 0) {
        ...
        SecurityItemsService.getSecurityItem(knexInstance, name)
          .then((results) => {
            if (results.length !== 0) {
              return res
                .status(400)
                .json({ error: "Security Item already exists!" });
            }
            // INSERT INTO SECURITY ITEMS TABLE

            SecurityItemsService.addSecurityItem(
              knexInstance,
              name,
              category
            ).then((results) => {
              ...
              ClientsService.getClients(knexInstance).then((results) => {
